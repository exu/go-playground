...
// operations. For more complex state we can use a mutex to safely access data across
// multiple goroutines.
...
func main() {
	...
	// To compare the mutex-based approach with another we’ll see later, ops will count how
	// many operations we perform against the state.
	...
	for r := 0; r < 100; r++ {
		go func() {
			...
			for {
				...
				// exclusive access to the state, read the value at the chosen key, Unlock()
				// the mutex, and increment the ops count.
	...
	// We’ll also start 10 goroutines to simulate writes, using the same pattern we did for
	// reads.
...
// Running the program shows that we executed about 3,500,000 operations against our
